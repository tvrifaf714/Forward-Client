...
async def Kanger(c: Client, m: Message):
    ...
    for i in range(len(Config.FORWARD_TO_CHAT_ID)):
        ...
        try:
            ...
            print(f"ForwardToChat: {ForwardToChat}")  # Debug print to see the value of ForwardToChat
            if not ForwardToChat:
                await m.edit(text=f"Sorry, can't get **Forward To Chat** for chat ID {Config.FORWARD_TO_CHAT_ID[i]}!")
                return 400
            if not ForwardToChat.permissions.can_send_messages:
                await m.edit(text=f"Sorry, you don't have permission to send messages in {ForwardToChat.chat.title} !")
                return 400
